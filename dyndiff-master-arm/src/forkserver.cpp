...
void callfunc(void* aFuncPtr, funcprototype aFuncType, char aArgv[][50], char **aRes){
    if(aFuncType == IFPP){
...
void callfunc_memcontent(void* aFuncPtr, funcprototype aFuncType, char aArgv[][50]){
    if(aFuncType == IFPP){
	func_ifpp_t tfuncptr;
        tfuncptr = (func_ifpp_t)aFuncPtr;
        LOGDEBUG("child processing");
